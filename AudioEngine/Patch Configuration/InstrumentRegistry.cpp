...
void InstrumentRegistry::parse_oscillator_config(const json& config, const float target_sample_rate, PatchDefinition* patch, SampleLoader* loader) {
    ...
    switch (type) {
        ...
        case OscillatorParsingType::SQUARE: {
            ...
            patch->square_oscillator_params = SquareOscillatorParams(440.0f, target_sample_rate);
            patch->oscillator_initialized = true;
            break;
        }
        case OscillatorParsingType::SAWTOOTH: {
            patch->oscillator_type = OscillatorType::SAWTOOTH;
            patch->sawtooth_oscillator_params = SawtoothOscillatorParams(440.0f, target_sample_rate);
            patch->oscillator_initialized = true;
            break;
        }
        case OscillatorParsingType::TRIANGLE: {
            patch->oscillator_type = OscillatorType::TRIANGLE;
            patch->triangle_oscillator_params = TriangleOscillatorParams(440.0f, target_sample_rate);
            patch->oscillator_initialized = true;
            break;
        }
...
void InstrumentRegistry::parse_svf_config(const json& config, const float target_sample_rate, PatchDefinition* patch, SampleLoader* loader) {
    ...
    patch->svf_filter_params.cutoff_multiplier = config.value("cutoff_multiplier", 4.0f);
    patch->svf_filter_params.resonance = config.value("resonance", 0.707f);
    patch->svf_filter_params.resonance_tracking = config.value("resonance_tracking", 0.0f);
